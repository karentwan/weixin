...
var data = {};
data.queryDate = moment().subtract("18", "hours").format("YYYY-MM-DD");
// 获取服务器端原始数据，交给initialData处理
var generateInitailData = function(call) {
    // initial setup data
    var cookies = generateCookieObj();

    var time = data.queryDate;
    console.log(time);
    requestConfig.data = {
        name: cookies.name,
        //		name: "万海燕",
...
var classSeparate = function(json) {
    ...
    // 2. 排序， 为去除重复项做准备
    classCache = classCache.sort(function(left, right) {
        return (left - right);
    });
...
var studentSeparate = function(json) {
    ...
    students = null;
    //sconsole.info("out func studentSeparate");
};
...
/*********************End 数据处理过程***************************************/
// 执行数据初始化
var dataInit = function(call) {
    // 存储UserName
    var cookies = generateCookieObj();
    //		console.log(cookies);
    data.username = cookies.name;
    generateInitailData(call);
};
